...
- (void)setItem:(NSMutableDictionary *)item {
    ...
    [_item setDictionary:item];

    if (item) {
        BOOL selected = [[item objectForKey:@"check"] boolValue];
...
-(void)setSelected:(BOOL)selected
{
    ...
    NSNumber *checkBool = [NSNumber numberWithBool:self.selected];
    [self.item setValue:checkBool forKey:@"check"];

    if (self.selected) {
        self.colorFill = _arrColor[self.tag%_arrColor.count];
        self.colorStroke = [UIColor clearColor];
        self.layer.shadowOpacity=1;

    }else{
        self.colorFill = [UIColor clearColor];
