...
- (void)setArrItem:(NSMutableArray *)arrItem {
    if (arrItem) {
        _arrItem = arrItem;
...
- (void)resetAllSegments {
    ...
    for (int i = 0; i< self.arrItem.count; i++) {
        ...
        FMYWNewsType *newsItem = self.arrItem[i];
        [btnItem setTitle:newsItem.name forState:UIControlStateNormal];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex {
    if (selectedIndex < self.muArrBtn.count) {
        ...
        if (self.currentBtn) {
            ...
            [self.currentBtn.titleLabel setFont:[UIFont systemFontOfSize:15]];
        }
        self.currentBtn = self.muArrBtn[selectedIndex];
        self.currentBtn.selected = YES;
        [self.currentBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:15]];
    }
}
