...
-(void)initArrCenter{
    basePoint = CGPointMake(self.frame.size.width/1.6, self.frame.size.height/2.85);
...
-(void)configureUIItems{
    ...
    for (int i=0; i<_arrTitle.count; i++) {
        ...
        controlBP.layer.shadowColor = [UIColor whiteColor].CGColor;
        controlBP.layer.shadowOffset=CGSizeMake(0.8, 0.8);
        controlBP.layer.shadowOpacity=0.1;
        controlBP.colorStroke = [UIColor whiteColor];
        ...
        if (_arrNewsItems && _arrNewsItems.count) {
            controlBP.item  = _arrNewsItems[i];
        }
...
-(void)controlBPClick:(Control_BezierPath*)cBP{
    ...
    if (self.CBPBlock) {
        self.CBPBlock(cBP.tag, cBP.item);
    }
}
...
- (void)setArrNewsItems:(NSArray *)arrNewsItems {
    _arrNewsItems = arrNewsItems;
    NSMutableArray *titleArr = [NSMutableArray array];
    for (NSDictionary *item in arrNewsItems) {
        NSString *title = [item objectForKey:@"name"];
        [titleArr addObject:title];
    }
    self.arrTitle = titleArr;
}
